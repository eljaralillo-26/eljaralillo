...
// Los archivos grandes de verdad (PDF de facturas) van al "almacén de
// archivos" de Firebase (Storage), no a la base de datos — ahí no hay
// límite de 1 MB, admite archivos de muchos MB sin problema.
async function subirArchivoStorage(ruta, file) {
  try {
    const res = await fetch(
      `https://firebasestorage.googleapis.com/v0/b/${FIREBASE_STORAGE_BUCKET}/o?uploadType=media&name=${encodeURIComponent(ruta)}`,
      {
        method: "POST",
    ...
    return `https://firebasestorage.googleapis.com/v0/b/${FIREBASE_STORAGE_BUCKET}/o/${encodeURIComponent(ruta)}?alt=media${
      token ? `&token=${token}` : ""
    }`;
  } catch {
    return null;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        ...
        // no había cuerpo JSON legible
      }
      console.error("Fallo al guardar en Firestore:", key, res.status, detalle);
      saveShared.ultimoError = `${res.status}: ${detalle}`;
      return false;
    }
    return true;
  } catch (err) {
    // demo: si falla el guardado, seguimos en memoria
    console.error("Fallo de red al guardar en Firestore:", key, err);
    saveShared.ultimoError = String(err?.message || err);
    return false;
  }
}

function descargarDataUrl(dataUrl, nombreArchivo) {
  // Si ya es un enlace real (subido a Firebase Storage), se abre tal cual —
  // no hace falta convertir nada, el navegador ya sabe descargarlo/verlo.
...
function localizarTablaGuia(wb) {
  ...
  for (const nombreHoja of nombresOrdenados) {
    ...
    for (let i = 0; i < Math.min(filas.length, 25); i++) {
      const filaTexto = filas[i].map((c) => String(c || "").trim().toLowerCase());
      const tieneDni = filaTexto.some((c) => c.length <= 25 && c.includes("dni"));
      const tieneKilos = filaTexto.some((c) => c.length <= 25 && c.includes("kilo"));
...
export default function App() {
  ...
  useEffect(() => {
    const id = setInterval(async () => {
      ...
      if (!editadoHaceNada) {
        ...
        const pd = await pollShared("jaralillo-pedidos");
        if (pd.hay) setPedidos(pd.value);
      }
  ...
  const eliminarAgricultor = useCallback(
    async (dniEliminar) => {
      ultimaEdicionRef.current = Date.now();
      const nuevosAgricultores = { ...agricultores };
      delete nuevosAgricultores[dniEliminar];
      setAgricultores(nuevosAgricultores);
      await saveShared("jaralillo-agricultores", nuevosAgricultores);

      const nuevosPerfiles = { ...perfiles };
      delete nuevosPerfiles[dniEliminar];
      setPerfiles(nuevosPerfiles);
      await saveShared("jaralillo-perfiles", nuevosPerfiles);

      agricultoresCountRef.current = Object.keys(nuevosAgricultores).length;
      ultimaEdicionRef.current = Date.now();
    },
    [agricultores, perfiles]
  );

  const importarClientes = useCallback(
    async (lista) => {
      ultimaEdicionRef.current = Date.now();
      const nuevo = { ...agricultores };
      let añadidos = 0;
      let yaExistian = 0;
      lista.forEach(({ dni, nombre, poblacion }) => {
        if (nuevo[dni]) {
          yaExistian++;
        } else {
          nuevo[dni] = { nombre, poblacion: poblacion || "" };
          añadidos++;
        }
      });
      setAgricultores(nuevo);
      await saveShared("jaralillo-agricultores", nuevo);
      agricultoresCountRef.current = Object.keys(nuevo).length;
      ultimaEdicionRef.current = Date.now();
      return { añadidos, yaExistian };
    },
    [agricultores]
  );
    ...
    <div style={styles.app}>
      ...
      {vista === "puesto" && (
        <VistaPuesto
          ...
          onEliminarAgricultor={eliminarAgricultor}
          presencia={presencia}
          precios={precios}
...
function FacturaCardAgricultor({ factura: f, onSubirFirmada }) {
  ...
  const subirFirmada = async (e) => {
    ...
    if (file.size > 8 * 1024 * 1024) {
      setError("El PDF pesa demasiado (máx. 8 MB).");
      e.target.value = "";
      return;
    }
    setSubiendo(true);
    const ruta = `facturas/${f.dni}/firmadas/${Date.now()}-${file.name}`;
    const url = await subirArchivoStorage(ruta, file);
    if (!url) {
      setSubiendo(false);
      e.target.value = "";
      setError("No se ha podido subir el archivo. Comprueba tu conexión e inténtalo de nuevo.");
      return;
    }
    const resultado = await onSubirFirmada(f.id, file.name, url);
    setSubiendo(false);
    e.target.value = "";
    if (resultado && resultado.ok === false) {
      setError(`No se ha podido guardar la factura firmada. (${saveShared.ultimoError || "motivo desconocido"})`);
    }
  };
...
function VistaPuesto({
  ...
  return (
    <div style={styles.screen}>
      ...
      {tab === "agricultores" && (
        <PanelAgricultores agricultores={agricultores} perfiles={perfiles} presencia={presencia} onReiniciarPin={onReiniciarPin} onEliminarAgricultor={onEliminarAgricultor} />
      )}
      ...
      {tab === "sincronizar" && (
        <PanelSincronizarGuia agricultores={agricultores} onSincronizar={onSincronizarGuia} onImportarClientes={onImportarClientes} />
      )}
...
function PanelAgricultores({ agricultores, perfiles, presencia, onReiniciarPin, onEliminarAgricultor }) {
  ...
  return (
    <div style={{ marginTop: 20 }}>
      <p style={styles.cardHint}>
        ...
        Si un agricultor ya no trabaja con el puesto, puedes eliminarlo: deja de poder entrar
        con su DNI, aunque su historial de albaranes y facturas se conserva.
      </p>
      <div style={styles.albaranList}>
          ...
          return (
            <div key={dniAg} style={styles.albaranRow}>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                {confirmandoBorrado === dniAg ? (
                  <div style={styles.confirmarRow}>
                    <button
                      style={styles.confirmarBtnSi}
                      onClick={() => {
                        onEliminarAgricultor(dniAg);
                        setConfirmandoBorrado(null);
                      }}
                    >
                      Sí, eliminar
                    </button>
                    <button style={styles.confirmarBtnNo} onClick={() => setConfirmandoBorrado(null)}>
                      Cancelar
                    </button>
                  </div>
                ) : registrado && confirmando === dniAg ? (
                  <div style={styles.confirmarRow}>
                    <button
                      style={styles.confirmarBtnSi}
                      onClick={() => {
                  ...
                  <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                    ...
                    <button
                      style={{ ...styles.linkBtnMini, color: "#B4552F" }}
                      onClick={() => setConfirmandoBorrado(dniAg)}
                    >
                      Eliminar
                    </button>
                  </div>
...
function PanelSincronizarGuia({ agricultores, onSincronizar, onImportarClientes }) {
  ...
  const [nombreArchivoClientes, setNombreArchivoClientes] = useState("");
  const [importando, setImportando] = useState(false);
  const [resultadoClientes, setResultadoClientes] = useState(null);

  const procesarArchivoClientes = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    setErrorClientes("");
    setResultadoClientes(null);
    setNombreArchivoClientes(file.name);
    ...
    lector.onload = async (ev) => {
      try {
        ...
        let encontrado = null;
        const nombresOrdenados = [...wb.SheetNames].sort((a, b) => {
          const aCli = /client/i.test(a) ? 0 : 1;
          const bCli = /client/i.test(b) ? 0 : 1;
          return aCli - bCli;
        });
        for (const nombreHoja of nombresOrdenados) {
          const hoja = wb.Sheets[nombreHoja];
          const filasHoja = XLSX.utils.sheet_to_json(hoja, { header: 1, defval: "" });
          for (let i = 0; i < Math.min(filasHoja.length, 25); i++) {
            const filaTexto = filasHoja[i].map((c) => String(c || "").trim().toLowerCase());
            const tieneDni = filaTexto.some((c) => c.length <= 25 && c.includes("dni"));
            const tieneNombre = filaTexto.some((c) => c.length <= 25 && c.includes("nombre"));
            if (tieneDni && tieneNombre) {
              const headers = filasHoja[i].map((c) => String(c || "").trim());
              const colDni = encontrarColumna(headers, ["dni"]);
              const colNombre = encontrarColumna(headers, ["nombre"]);
              const colPoblacion = encontrarColumna(headers, ["poblac", "municipio", "localidad"]);
              const datos = filasHoja[i + 1] !== undefined ? filasHoja.slice(i + 1) : [];
              const lista = datos
                .map((f) => {
                  const obj = {};
                ...
                .filter((f) => {
                  const dniTexto = String(f[colDni] || "").trim();
                  return dniTexto !== "" && dniTexto.toUpperCase() !== "DNI";
                })
                .map((f) => ({
                  dni: String(f[colDni]).trim().toUpperCase(),
                  nombre: colNombre ? String(f[colNombre] || "").trim() : "",
                  poblacion: colPoblacion ? String(f[colPoblacion] || "").trim() : "",
                }))
                .filter((c) => c.dni && c.nombre);
              encontrado = lista;
              break;
            }
          }
        ...
        if (!encontrado || encontrado.length === 0) {
          setErrorClientes("No encuentro una tabla con columnas de DNI y Nombre en ninguna hoja.");
          return;
        }
        setImportando(true);
        const res = await onImportarClientes(encontrado);
        setImportando(false);
        setResultadoClientes(res);
      } catch {
        setErrorClientes("No he podido leer el archivo. Comprueba que es un Excel (.xlsx) o CSV válido.");
      }
    };
  ...
  return (
    <div style={{ marginTop: 20 }}>
      <div style={styles.campañaCard}>
        <p style={styles.kgLabel}>Importar toda la lista de clientes de golpe</p>
        <p style={styles.tinyHintLeft}>
          Sube el mismo archivo con la hoja "CLIENTES" (Código, Nombre, DNI, Dirección,
          Población) y se da de alta a todos los que no estén ya. No hace falta repetirlo
          — solo la primera vez, o cuando tengas clientes nuevos que aún no hayan traído
          ningún albarán.
        </p>
        <label style={{ ...styles.subirBtn, marginTop: 10, opacity: importando ? 0.6 : 1 }}>
          {importando ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />}
          {nombreArchivoClientes || "Subir Excel de clientes"}
          <input
            type="file"
            accept=".xlsx,.xls,.csv"
            onChange={procesarArchivoClientes}
            style={{ display: "none" }}
            disabled={importando}
          />
        </label>
        {errorClientes && (
          <p style={styles.errorText}>
            <AlertTriangle size={13} style={{ flexShrink: 0, marginTop: 1 }} /> {errorClientes}
          </p>
        )}
        {resultadoClientes && (
          <p style={styles.tinyHintLeft}>
            <strong>{resultadoClientes.añadidos}</strong> clientes nuevos dados de alta
            {resultadoClientes.yaExistian > 0 ? ` · ${resultadoClientes.yaExistian} ya existían` : ""}.
          </p>
        )}
      </div>

      <p style={{ ...styles.cardHint, marginTop: 22 }}>
...
function PanelFacturasPuesto({ agricultores, facturas, onSubir, onVaciar }) {
  ...
  const subirArchivo = async (e) => {
    ...
    if (file.size > 8 * 1024 * 1024) {
      setError("El PDF pesa demasiado (máx. 8 MB). Comprímelo e inténtalo de nuevo.");
      e.target.value = "";
      return;
    }
    setSubiendo(true);
    const ruta = `facturas/${dniElegido}/${Date.now()}-${file.name}`;
    const url = await subirArchivoStorage(ruta, file);
    if (!url) {
      setSubiendo(false);
      e.target.value = "";
    ...
    const resultado = await onSubir(dniElegido, file.name, url);
    setSubiendo(false);
    e.target.value = "";
    if (resultado && resultado.ok === false) {
      setError(`No se ha podido guardar la factura. (${saveShared.ultimoError || "motivo desconocido"})`);
    }
  };
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      <div style={styles.avisoEspacioBox}>
        <p style={styles.tinyHintLeft}>
          Las facturas ahora se guardan en el almacén de archivos de Firebase (hasta 8 MB por
          PDF), no en la base de datos — así que ya no hay problema de espacio compartido entre
          todas. Este botón queda por si alguna vez quieres limpiar las de prueba.
        </p>
        {confirmandoVaciar ? (
          <div style={styles.confirmarRow}>
            <button
